...
const static int kBytesToReserve = 64000;
...
void *IncreaseFreeListSize() {
	...
	long pointer_value = (long) temp_previous_program_break;
	...
	// Update how much unallocated space I now have, before you align the
	// memory
	unallocated_bytes_in_data_segment += kBytesToReserve;

	// TODO: Align the memory you just called sbrk(2) for.
	if ((pointer_value % kSixteen) != 0) {
		fputs("!!!!!!!!!  unaligned memory", stderr);
	}
